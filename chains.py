import os
from langchain_groq import ChatGroq
from langchain_core.prompts import PromptTemplate
from langchain_core.output_parsers import JsonOutputParser
from langchain_core.exceptions import OutputParserException

class Chain:
    def __init__(self):
        self.llm = None  # Initialize lazily

    def _init_llm(self):
        if self.llm is None:
            groq_api_key = os.environ.get("GROQ_API_KEY")
            if not groq_api_key:
                raise ValueError("GROQ_API_KEY is missing in Streamlit secrets!")
            self.llm = ChatGroq(
                temperature=0,
                groq_api_key=groq_api_key,
                model_name="llama-3.3-70b-versatile"
            )

    def extract_jobs(self, cleaned_text):
        self._init_llm()
        prompt_extract = PromptTemplate.from_template(
            """
            ### SCRAPED TEXT FROM WEBSITE:
            {page_data}
            ### INSTRUCTION:
            Extract job postings as JSON with keys: role, experience, skills, description.
            Only return valid JSON.
            """
        )
        chain_extract = prompt_extract | self.llm
        res = chain_extract.invoke({"page_data": cleaned_text})
        try:
            json_parser = JsonOutputParser()
            res = json_parser.parse(res.content)
        except (OutputParserException, ValueError) as e:
            raise OutputParserException(f"Error parsing JSON: {e}")
        return res if isinstance(res, list) else [res]

    def extract_skills(self, text):
        self._init_llm()
        prompt_extract_skills = PromptTemplate.from_template(
            """
            ### TEXT:
            {text}
            ### INSTRUCTION:
            Extract the most relevant technical skills as comma-separated keywords.
            """
        )
        chain_extract_skills = prompt_extract_skills | self.llm
        res = chain_extract_skills.invoke({"text": text})
        return res.content.split(",")

    def calculate_match_score(self, resume_skills, job_skills):
        resume_skills = set([s.strip().lower() for s in resume_skills])
        job_skills = set([s.strip().lower() for s in job_skills])
        matched_skills = resume_skills.intersection(job_skills)
        return (len(matched_skills) / len(job_skills)) * 100 if job_skills else 0

    def write_mail(self, job, links):
        self._init_llm()
        prompt_email = PromptTemplate.from_template(
            """
            ### JOB DESCRIPTION:
            {job_description}

            ### INSTRUCTION:
            Write a concise, professional cold email for this job. Highlight relevant skills and experience.

            ### EMAIL (NO PREAMBLE):
            """
        )
        chain_email = prompt_email | self.llm
        res = chain_email.invoke({"job_description": str(job), "link_list": links})
        return res.content
